...
public class CameraManager : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKey (KeyCode.Space) && (GameManager.instance.getSelectedObject() != null)) {
			...
			gameObject.transform.position = new Vector3 (selectedObjectPosition.x, gameObject.transform.position.y, selectedObjectPosition.z - 3);
		} else {
			...
			if (Input.GetKey (KeyCode.D) || (viewportPoint.x > (1 - mouseMovementPadding))) {
				gameObject.transform.Translate (new Vector3 (Mathf.Max (horizontalScroll, minScrollSpeed), 0, 0));
			}
			if (Input.GetKey (KeyCode.A) || (viewportPoint.x < mouseMovementPadding)) {
				gameObject.transform.Translate (new Vector3 (Mathf.Min (horizontalScroll, -minScrollSpeed), 0, 0));
			}
			if (Input.GetKey (KeyCode.W) || (viewportPoint.y > (1 - mouseMovementPadding))) {
				gameObject.transform.Translate (new Vector3 (0, Mathf.Max (verticalScroll, minScrollSpeed), 0));
			}
			if (Input.GetKey (KeyCode.S) || ((viewportPoint.y < mouseMovementPadding * 3) && (viewportPoint.y > mouseMovementPadding * 2))) {
				gameObject.transform.Translate (new Vector3 (0, Mathf.Min (verticalScroll, -minScrollSpeed), 0));
			}
			if (zoom != 0) {
